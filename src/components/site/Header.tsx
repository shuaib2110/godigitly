  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? "py-2" : "py-4"}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className={`flex items-center justify-between rounded-full px-4 sm:px-6 py-3 transition-all ${scrolled ? "glass shadow-sm" : "bg-transparent"}`}>
          ...
          <div className="hidden lg:flex items-center gap-3">
            <Link to="/contact" className="px-5 py-2.5 rounded-full bg-foreground text-background text-sm font-semibold hover:opacity-90 transition">
              Book a call →
            </Link>
          </div>
        ...
        {open && (
          <div className="lg:hidden mt-2 rounded-3xl glass p-4 animate-fade-in">
            <nav className="flex flex-col gap-1">
              {nav.map((n) => (
                <Link key={n.to} to={n.to} className="px-3 py-2.5 rounded-xl hover:bg-foreground/5 text-base font-medium">
                  {n.label}
                </Link>
              ))}
              <Link to="/contact" className="mt-2 text-center px-5 py-3 rounded-full bg-foreground text-background font-semibold">
                Book a call →
              </Link>
            </nav>
          </div>
