...
export const Route = createFileRoute("/case-studies")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "See how GoDigitly scaled D2C, SaaS and service brands with performance marketing — real ROAS, real revenue, real campaigns." },
...
const cases = [
  {
    ...
    summary: "Rebuilt the creative system around UGC + founder content, restructured Meta into ASC+ scaling and added click-to-WhatsApp for high-intent retargeting.",
  },
  {
    brand: "FitForge",
    ...
    summary: "Launched a 7-day creative testing cadence, built a free trial → WhatsApp nurture funnel and added Performance Max for capture demand.",
  },
  {
    brand: "Aurora Living",
    ...
    summary: "Built launch funnel from scratch — landing page, offer, ads system, email/WhatsApp flows. Hit ₹1Cr/month by month 6.",
  },
];
...
function Counter({ to, prefix = "", suffix = "", decimals = 0 }: { to: number; prefix?: string; suffix?: string; decimals?: number }) {
  ...
  return (
    <span ref={ref} className="tabular-nums">
      ...
      >
        {inView ? (
          <CountUp end={to} decimals={decimals} />
        ) : (
          (0).toFixed(decimals)
        )}
      </motion.span>
...
function CountUp({ end, decimals = 0, duration = 1.4 }: { end: number; decimals?: number; duration?: number }) {
  const ref = useRef<HTMLSpanElement>(null);
...
function GrowthChart({ data }: { data: number[] }) {
  ...
  return (
    <svg ref={ref} viewBox={`0 0 ${w} ${h}`} className="w-full h-32">
      ...
      <motion.path d={area} fill="url(#lg)" initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.6 }} />
      <motion.path
        d={path}
        fill="none"
    ...
    <>
      <section className="relative overflow-hidden grain">
        ...
        <div className="mx-auto max-w-7xl px-6 pt-16 pb-20 sm:pt-24">
          <Reveal>
            <p className="text-sm uppercase tracking-widest text-violet font-semibold">Case studies</p>
            ...
            <article className="rounded-[2rem] border border-border bg-card overflow-hidden">
              <div className="grid md:grid-cols-12">
                <div className="md:col-span-5 p-8 sm:p-10 border-b md:border-b-0 md:border-r border-border">
                  <div className="text-xs uppercase tracking-widest text-violet font-semibold">{c.industry}</div>
                  <h2 className="mt-3 font-display text-3xl sm:text-4xl font-bold tracking-tight">{c.brand}</h2>
                  <p className="mt-4 text-2xl font-display font-semibold text-balance">{c.headline}</p>
                ...
                <div className="md:col-span-7 p-8 sm:p-10 bg-foreground/[0.02]">
                  <div className="grid grid-cols-3 gap-4">
                    {(["roas", "cac", "rev"] as const).map((k) => {
                      ...
                      const before = c.before[k]; const after = c.after[k];
                      return (
                        <div key={k}>
                          <div className="text-xs uppercase tracking-widest text-muted-foreground">{labels[k]}</div>
                          <div className="mt-2 grid grid-cols-2 gap-1">
                            <div>
                              <div className="text-xs text-muted-foreground">Before</div>
                              <div className="font-display text-xl font-semibold opacity-50">{before}{k === "roas" ? "×" : ""}</div>
                            </div>
                            <div>
                              <div className="text-xs text-violet font-semibold">After</div>
                              <div className="font-display text-2xl font-bold">
                                <Counter to={after} decimals={k === "roas" ? 1 : 0} suffix={k === "roas" ? "×" : ""} />
                              </div>
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="mt-8 rounded-2xl bg-background border border-border p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div className="text-xs uppercase tracking-widest text-muted-foreground">Monthly revenue (₹L)</div>
                      <div className="flex items-center gap-1 text-xs text-violet font-semibold"><TrendingUp className="size-3" /> +{Math.round((c.after.rev / Math.max(1, c.before.rev) - 1) * 100) || 100}%</div>
                    </div>
                    <GrowthChart data={c.chart} />
                  </div>

                  <Link to="/contact" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold underline-grow">
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        <Reveal>
          <div className="rounded-[2rem] bg-foreground text-background p-10 sm:p-16 text-center">
            ...
            <Link to="/contact" className="mt-8 inline-flex items-center gap-2 rounded-full bg-background text-foreground px-7 py-4 font-semibold">
